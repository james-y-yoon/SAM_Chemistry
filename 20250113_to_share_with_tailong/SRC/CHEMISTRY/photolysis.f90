module photolysis
    ...
    use chemistry_params, only : do_O3_photolysis, &
       do_NO2_photolysis, do_H2O2_photolysis, do_HCHO_photolysis, do_NO3_photolysis, &
       do_ISOPOOH_photolysis
    
    use cloudchem_Parameters, only : ind_NO2, ind_O3, ind_OH, ind_H2O2, &
        ind_HCHO, ind_HO2, ind_CO, ind_NO, ind_NO3, ind_ISOPOOH
    
    CONTAINS
    
    subroutine photolysis_driver(gchem_field, M_profile)
        ...
        if (do_O3_photolysis) then
            photolysis_rate = calculate_photolysis_rate_O3(M_profile)
    ...
    function calculate_radiation_flux() result(radiation_flux)
        ...
        use rad, only : insolation_TOA, swDownSurface, swDown
        use grid, only : z, dz, nz, dtn, nzm
        ...
        if ( do_linear_interpolation ) then
            ...
            if ( allocated(swDown) ) then
                do i = 1, nx
                    do j = 1, ny
                        do k = 1, nzm
                            radiation_flux(i,j,k) = swDown(i,k)
                            ! if ( radiation_flux(i,j,k) .gt. 0 ) then
                            !     print*, 'Radiative flux = ', radiation_flux(i,j,k)
                            ! endif
                        enddo
                    enddo
                enddo
    ...
    function calculate_photolysis_rate_O3(M_profile) result(photolysis_rate)
        ! Calculates the photolysis loss frequency for a species
        use vars, only : qv0
        implicit none
