...
static void print_stats_line (FILE *, struct Stats *, struct Stats *);
...
int t_pr (int procid)
{
  ...
  struct Stats stats0;      /* per timer stats */
  ...
  */
  ...
  sprintf (outfile, "%s%d","timing.", procid);
  ...
  */
  ...
  for (thread = 0; thread < numthreads; thread++) {
    ...
    */
	...
	fillstats (&stats, ptr);
        fillstats (&stats0, timers[thread]);
	...
	*/
	...
	print_stats_line (fp, &stats, &stats0);
      }
    }
  ...
  if (numthreads > 0 && timers[1] != NULL) {
    ...
    */
	...
	for (tptr = timers[thread]; tptr != NULL; tptr = tptr->next) {
      ...
      strcat (name, ".sum");
      fprintf (fp, "%-19s", name);
      print_stats_line (fp, &threadsum,&stats0);
      fprintf (fp, "\n");
...
void print_stats_line (FILE *fp, struct Stats *stats, struct Stats *stats0)
{
  int index;
  int n;
...
*/
  ...
  if (wallenabled)
/*    fprintf (fp, "%9.3f %9.3f %9.3f ", stats->elapse, stats->max_wtime, stats->min_wtime);
*/
    fprintf (fp, " %9.3f        %9.3f", stats->elapse, stats->elapse/stats0->elapse*100.);
...
void print_header (FILE *fp, int indent_level)
{
  ...
  if (wallenabled)
    fprintf (fp, "   Wallclock    Percent of total  ");

  for (n = 0; n < nevent; n++)
    if (event[n]->name > pcl_start && event[n]->name <= pcl_end)
      fprintf (fp, "%s", event[n]->string);
