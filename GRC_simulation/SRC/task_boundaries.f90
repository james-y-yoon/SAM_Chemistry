...
!!$     .or. doprecip.and.flag_precip(i).eq.1 ) &
 ...
 do i = 1,ngchem_fields
   call task_exchange(gchem_field(:,:,:,i), dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm, &
                   3+NADVS,3+NADVS,3+NADVS,3+NADVS,4+nsgs_fields+nsgs_fields_diag+nmicro_fields+ntracers+i)
 end do

 do i = 1,naqchem_fields
   call task_exchange(aqchem_field(:,:,:,i), dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm, &
                   3+NADVS,3+NADVS,3+NADVS,3+NADVS,4+nsgs_fields+nsgs_fields_diag+nmicro_fields+ntracers+ngchem_fields+i)
  end do

   ! at the moment the number of aqchem_gasprod_fields is identical to the number of aqchem_fields
  do i = 1,naqchem_fields
     call task_exchange(aqchem_gasprod_field(:,:,:,i), dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm, &
  ...
  do i = 1,narchem_fields
     call task_exchange(archem_field(:,:,:,i), dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm, &
...
!!$     .or. doprecip.and.flag_precip(i).eq.1 ) &
 ...
 if(dotracers) then
  ...
  do i=1,naqchem_fields
     call task_exchange(aqchem_gasprod_field(:,:,:,i),dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm, &
               1,1,1,1,4+nsgs_fields+nsgs_fields_diag+nmicro_fields+ntracers+ngchem_fields+naqchem_fields+i)
  end do
  do i=1,narchem_fields
     call task_exchange(archem_field(:,:,:,i),dimx1_s,dimx2_s,dimy1_s,dimy2_s,nzm, &
