module chemistry
  ...
  use chem_isoprene_Parameters, only: NVAR, NSPEC, NFIX, ind_Isoprene, ind_IEPOX, ind_NO, ind_ISOPOOH, indf_OH, ind_ISOP1Nit, ind_ISOPDiNit, NREACT  ! NSPEC=NVAR+NFIX
...
subroutine chem_flux()  
  use grid, only : day, time, dt
  use vars, only : precsfc, tabs, interactive_soil_wetness ! precsfc (x,y), tabs (x,y,z) (JY)
  use rad, only : swDown ! for PAR
  ...
  !     fluxbch(:,:,ind_Isoprene) =  frac_of_peak * Iso_avg_flux * (pi/2.) * temperature_activity_factor
  ! else
  !     fluxbch(:,:,ind_Isoprene) = Iso_avg_flux * temperature_activity_factor
  ! end if
  ...
  ! Temperature and Radiation Activity Factors
  ...
  allocate(temperature_activity_factor(nx, ny), soil_NOx(nx, ny), ppfd(nx, ny), radiation_activity_factor(nx, ny))
  ...
  deallocate(soil_NOx, temperature_activity_factor, ppfd, radiation_activity_factor)
